...
let txtObj = {
	name: 'txtForm',
	render: function () {
		let txtGridSec = new azulInpGrid({rows: 1, cols: 1, name: 'txtarea'});
		let txtgrid = txtGridSec.inpgrid;

		this.inp1 = new azulTxtInp({Field: 'Notes', Length: '250px', cols: 80, rows: 10, idx: 1});

		let firstRow = txtgrid.els[0][0];
		firstRow.appendChild(this.inp1.getInpEl());

		return txtGridSec.inpgridDiv;
	},

	getInpValues: function() {
		let inpVal = {
			Txt: txtObj.inp1.getInpValue(),
		};
		const inpJsonStr = JSON.stringify(inpVal)
		console.log('txt inp: ' + inpJsonStr);
		return inpJsonStr;
	},
}
...
let txtSec = txtObj.render();
azul.docbody.appendChild(txtSec);
